...
There are **8 achievements** in total. Each achievement can only be unlocked once per player.
...
| all_quests    | Quest Master         | complete all 5 quests                        |

### Achievement Check Instructions

Run achievement checks after any state-changing event (XP gained, gold changed, lines written, quest completed). For each achievement not yet unlocked:
...
For `all_quests`: check after every quest completion — trigger when `quests_completed` count equals 5.
...
Lifetime counters (`total_xp_earned`, `lines_written`, etc.) used for achievement triggers are **never** reset.
