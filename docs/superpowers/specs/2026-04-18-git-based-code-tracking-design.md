...
**Author filtering:** The hook only processes commits by the current git user (`git config user.name`). Commits from other contributors (e.g., pulled in via merge) are skipped. This works correctly for AI-assisted workflows — when Claude or other AI tools write code, the git author is still the user.

**Claude session catch-up:** On session start (when DevQuest is enabled), Claude runs:
```
git log --author="<user>" --oneline <last_tracked_commit>..HEAD
```
...
**Disabled state:** When DevQuest is disabled, the hook exits silently. Commits made while disabled are not retroactively tracked on re-enable. This is intentional — disabled means frozen.

### AI-Generated Code and the Economy

Code generated by Claude in-conversation costs gold and earns no in-conversation XP/gold (the code generation gate). If that code later appears in a git commit, the hook will award XP/gold for the lines. This is an accepted trade-off:

- **Gold is the scarce resource**, not XP. The gold cost is the real gate.
- **Code generation costs 1.0 gold/line; commits earn 0.5 gold/line.** Even if AI-generated code is committed, the user nets -0.5 gold per line — the economy is self-correcting.
- Tracking which specific lines were AI-generated vs user-written at the commit level would be fragile and complex.
- The core loop is preserved: earn gold by writing code → spend gold on AI generation.
